...
from coding_agent.agents.default import AgentConfig, DefaultAgent, LimitsExceeded, NonTerminatingException, Submitted

console = Console(highlight=False)
prompt_session = PromptSession(history=FileHistory("interactive_history.txt"))
...
class InteractiveAgent(DefaultAgent):
    ...
    def add_message(self, role: str, content: str, **kwargs):
        ...
        if role == "assistant":
            console.print(
                f"\n[red][bold]agent[/bold] (step [bold]{self.model.n_calls}[/bold], [bold]${self.model.cost:.2f}[/bold]):[/red]\n",
                end="",
                highlight=False,
            )
        else:
